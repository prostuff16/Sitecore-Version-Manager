...
namespace Sitecore.VersionManager.sitecore_modules.Shell.VersionManager
{
    ...
    {
        public static readonly ID[] ConfigVersionTemplates = Settings.GetSetting("VersionManager.ValidTemplates", "").Trim().Replace(" ", "").Split(new char[] { ',' }, System.StringSplitOptions.RemoveEmptyEntries).Select(i => Guid.Parse(i).ToID()).ToArray();
        ...
        public static readonly ID[] IgnoredFolders = Settings.GetSetting("VersionManager.IgnoredFolders", "").Trim().Replace(" ", "").Split(new char[] { ',' }, System.StringSplitOptions.RemoveEmptyEntries).Select(i=>Guid.Parse(i).ToID()).ToArray();
        public static readonly XmlNodeList Roots = Factory.GetConfigNodes("settings/setting[@name='VersionManager.Roots']/root");
        public static readonly bool ServiceRunInDebugMode = Settings.GetBoolSetting("VersionManager.ExecuteServiceRunInDebugMode", true);
